...
void Andromeda::Gui::Panels::drawViewportGUI(GuiRenderer& guiRenderer,ViewportDrawInfo& drawInfo)
{
    ...
    if (drawInfo.ImGuiMouseX) *drawInfo.ImGuiMouseX = -1.0f;
    if (drawInfo.ImGuiMouseY) *drawInfo.ImGuiMouseY = -1.0f;
    ImGui::Image((void*)(intptr_t)drawInfo.framebufferTexture, currentSize, ImVec2(0, 1), ImVec2(1, 0));
    ...
    if (hovered) {
        ...
        if (drawInfo.ImGuiMouseX) *drawInfo.ImGuiMouseX = rel.x;
        if (drawInfo.ImGuiMouseY) *drawInfo.ImGuiMouseY = rel.y;
    }
    else {
